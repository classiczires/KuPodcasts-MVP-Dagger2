...
public class MostPupolarHorizontalAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private final int VIEW_TYPE_ITEM = 0;
    private final int VIEW_TYPE_LOADING = 1;

    private List<Podcast> mostPopular;
    Context ctx;


    public MostPupolarHorizontalAdapter(List<Podcast> mostPopular, Context ctx) {
        this.mostPopular = mostPopular;
        this.ctx = ctx;
    }

    public int getItemViewType(int position) {
        return mostPopular.get(position) == null ? VIEW_TYPE_LOADING : VIEW_TYPE_ITEM;
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof MainActivity.GridViewHolder) {
            ...
            Podcast podcast = mostPopular.get(position);
