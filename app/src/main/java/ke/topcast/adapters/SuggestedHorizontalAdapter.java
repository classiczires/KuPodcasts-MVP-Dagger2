...
public class SuggestedHorizontalAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private final int VIEW_TYPE_ITEM = 0;
    private final int VIEW_TYPE_LOADING = 1;
    private List<Podcast> suggestedPodcasts;
    Context ctx;


    public SuggestedHorizontalAdapter(List<Podcast> suggestedPodcasts, Context ctx) {
        this.suggestedPodcasts = suggestedPodcasts;
        this.ctx = ctx;
    }

    public int getItemViewType(int position) {
        return suggestedPodcasts.get(position) == null ? VIEW_TYPE_LOADING : VIEW_TYPE_ITEM;
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof MainActivity.GridViewHolder) {
            ...
            Podcast podcast = suggestedPodcasts.get(position);
